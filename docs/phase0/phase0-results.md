...
## 实验 #5: Phase 0.5 单篇文案真实生成闭环

- **日期**: 2026-05-02
- **状态**: ✅ 完成
- **内容**:
  - 文案列表触发真实 bf16 生成
  - 按段串行生成，生成音频保存到 App 持久化目录
  - 导出完整 WAV
  - App 退出后重新打开，已生成文案仍可再次导出
  - 生成失败时可看到失败状态
- **用户验收结果**:
  - ✅ 单音色生成音频通过
  - ✅ 多角色/多段音频生成通过
  - ✅ App 退出后重新打开仍可再次导出
  - ⏳ 删除文案后是否清理关联音频资产暂未测试，后续交给 AI 自动/手动补测
  - ✅ 失败状态可见
- **发现问题**:
  - ❌ 音色不可控：虽然角色音色绑定 UI 能保存，但生成链路未真正使用绑定音色。
  - ❌ 绑定音色不能试听。
  - ❌ 单音色输出和多音色输出目前仍表现为随机音色，不能作为多音色 MVP 通过依据。
- **决策**:
  - Phase 0.5 的"真实生成与导出闭环"通过。
  - "音色控制"不在 Phase 0.5 内继续硬修，升级为 Phase 2 前置验证项。
  - 后续必须先证明模型层支持稳定的 voice/refAudio/refText 控制，再把角色绑定接入正式生成。
...
- [x] 音频保存为 WAV 文件
- [x] 音频可以播放
- [ ] 支持不同声音切换（Phase 0.5 发现当前不可控，需 Phase 2 前置验证）
...
| 2026-05-02 | 角色音色绑定未影响真实生成，输出音色随机 | Phase 2 前先做 voice/refAudio/refText 可控性验证，再接入角色绑定 | ⏳ 待解决 |
| 2026-05-02 | 删除文案后是否清理关联音频资产未测试 | 后续补自动/手动验收 | ⏳ 待验证 |
...
5. [x] 运行真实测试验证音频生成
6. [x] 记录性能和质量结果
7. [x] 替换 GenerationService 的占位实现
8. [x] 集成到文案生成流程
9. [ ] 验证音色控制能力（voice instruct / refAudio / refText）
10. [ ] 将角色音色绑定接入真实生成链路
